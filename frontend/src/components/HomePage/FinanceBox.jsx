...
const DonutChart = () => {
  ...
  const containerStyle = {
    position: 'relative',
    width: '300px', // Adjust the width of the container div
    height: '300px', // Adjust the height of the container div
...
const FinanceBox = () => {
  return (
    ...
    <div style={{display:'flex'}}>
      ...
      <p style={{fontSize: '18px'}}>1 May 2023 - 31 May 2023</p>
      <p style={{fontSize: '70px', fontWeight:'500'}}>$678.14</p>
    ...
    <div className='scroll-container' style={{
      ...
      {financeData && financeData.map(item => (
        <div style={{display:'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 10px'}}>
        <div>
          <p style={{fontSize: '23px'}}>{item.description}</p>
            <p style={{color: '#A1A1A1'}}>{formatDateTime(item.dateTime)}</p>
          </div>
          <p style={{color:'red', fontSize: '23px', fontWeight: '600'}}>SGD -11.60</p>
        </div>
      ))}
    </div>
    </div>
